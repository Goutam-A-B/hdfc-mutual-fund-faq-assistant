...
| 7.12 | Embedding quota hit on a full re-index | Free-tier limits during a weekday rebuild — and CI starts each run with no embedding cache (the cache is gitignored), so every run re-embeds all ~170 chunks | Backoff/resume already implemented; `gemini-embedding-001` free-tier RPD comfortably fits one full re-index. Run #25963006168 confirmed 171/171 embeds in ~12 min within quota | HTTP 429 during `3-build-index` |
